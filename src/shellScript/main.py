            ...
            for line in f.readlines():
                ...
                for rule in ANALYSIS_RULES:
                    for err_code in self.errs_dict[rule]:
                        if (info[0] == err_code):
                            for i in range(len(self.codestyle_lint_json[rule])):
                                if (err_code in self.codestyle_lint_json[rule][i]):
                                    self.codestyle_lint_json[rule][i][err_code] += 1
                                    break
                            else:
                                self.codestyle_lint_json[rule].append({'{_code}'.format(_code=err_code): 1})
                            break
                    else:
                        continue
                    break
